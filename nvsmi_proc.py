...
# nvidia-smi extension to show processes inside docker containers
...
def query_gpu():
    ...
    temperature_gpu_info = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:]
    ...
    utilization_gpu_info = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:]
    ...
    utilization_memory_info = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:]
    ...
    for i, gpu in enumerate(gpus):
        ...
        pid_info = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:]
        if len(pid_info)==0:
            pid = 0
        else:
            pid = int(pid_info[0])
    ...
    for i, gpu in enumerate(gpus):
        ...
        if pid ==0:
            ...
            ps_info_str = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:][0]
            ...
            container_id_str = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][1:]
            ...
            container_name_str = sp.check_output(command.split()).decode('ascii').split('\n')[:-1][0]
